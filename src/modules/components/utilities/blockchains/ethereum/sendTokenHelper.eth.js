...
const transferEthToken = async (token, toAddress, amount, userId, blockchain) => {
    ...
    try {
        ...
        if (token.toLowerCase() === "eth") {
            txData = "";
            tokenAddress = "";
            const userBalance = await EthWalletDetails.getUserEthWalletBalance(userAddress, false);
        ...
        if (status === 404) {
            const response = constructConversation(gasEstimateResponse, null, false, false);
            conversationResponses.push(response)
            return conversationResponses
        }
        ...
        const message = `You are about to transfer ${data.rawAmount} ${data.token
        ...
        const response = constructConversation(result, finalData, true, false, 60000);
        conversationResponses.push(response);
