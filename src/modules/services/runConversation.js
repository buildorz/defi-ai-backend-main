...
const { createRestakeTransaction } = require("../components/eigenlayer/index.js");
...
async function runConversation(userId, message, blockchain) {
  ...
  try {
    ...
    //  check if the message contains standard operations
    ...
    // if (
    //   ["polygon", "pol"].includes(
    //     blockchain.toLowerCase() && !standardOperations.includes(message)
    //   )
    // ) {
    //   const response = await axios.post(
    //     "https://flow-api.mira.network/v1/flows/flows/venmus/polygon-stack?version=0.0.1",
    //     {
    //       question: message,
    //     },
    //     {
    ...
    //         miraauthorization: ENVIRONMENT.MIRA_POLYGON_STACK.API_KEY,
    //       },
    //     }
    //   );

    //   if (![200, 201].includes(response?.status) || !response?.status) {
    //     const message = "Failed to retrieve users message";
    //     const reply = constructConversation(message, null, false, false);
    //     queryResponse.push(reply);
    //     return queryResponse;
    ...
    //   const reply = constructConversation(
    ...
    if (blockchain.toLowerCase() === "polygon") {
      ...
      polygonUserMessages.push({
        role: "assistant",
        content: response?.data?.result,
        refusal: null,
      });

      console.log("firstMessage: ", polygonUserMessages);

      const response2 = await openai.chat.completions.create({
        model: gptModel,
        messages: polygonUserMessages,
      });
      ...
      const reply = constructConversation(content, null, false, false);
    ...
    if (responseMessage.tool_calls) {
      ...
      const availableFunctions = {
        ...
        check_base_name_availability: checkBaseNameAvailability,
        purchase_base_name: purchaseBaseName,
        restake: createRestakeTransaction
      };
